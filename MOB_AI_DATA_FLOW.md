...
This document describes how `src/mob_ai.rs` separates game-thread work from worker-pool work. Keep this model intact when changing Mob AI.
...
1. `MobAiState::run_tick` runs from `ServerTickStartEvent`.
...
- remove the UUID from `active_path_jobs`
...
- remove the UUID from `active_velocity_jobs`
...
- `apply_planned_velocity` removes a `VelocityPlan` from `planned_velocities`.
...
- `workers.rs`: job submission, active-job bookkeeping, result publication.
...
The invariant after refactor: worker modules may depend on owned data types, but must not depend on live Pumpkin entity/world mutation.
