//! Defense and Acrobatics: damage-taken XP and bounded damage reductions.
//!
//! XP is computed from the incoming damage *before* perk reductions, so a
//! stronger defense never slows its own progression. Reductions are
//! multiplicative, capped per skill, and can never amplify damage.
...
pub async fn handle_entity_damage(
    ...
    if is_fall {
        // Acrobatics XP from fall damage (before reductions).
        ...
        if fall_xp > 0 {
            progression::award_xp(state, &player, SkillId::Acrobatics, fall_xp, XpSource::Fall)
                .await;
        }
    }
    ...
    // Bounded reductions: Acrobatics roll first, then Defense resilience.
    if is_fall {
        let level = player_level(state, &player, SkillId::Acrobatics).await;
